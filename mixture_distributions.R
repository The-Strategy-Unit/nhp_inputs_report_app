...
strategy_lookup <- read.csv('strategy_lookup.csv',
                            header = TRUE)

# 2 Wrangle data ----
# Create a dataframe of the normal distribution parameters for each peer and
# activity subset:
normal_dists <- data |>
  ...
  dplyr::select(peer, activity_subset = strategy, mu, sigma)
...
for (i in (activity_subsets)) {
  ...
  for (j in (peers)) {
    ...
    peer_dist <- distr::Norm(mean = norm_param$mu,
                             sd = norm_param$sigma)
...
peer_agg_ecdf_pdf <- data.frame(activity_subset = character(),
                               q = numeric(),
                               ecdf_value = numeric(),
                               pdf_value = numeric())
...
peer_agg_p10_p50_p90 <- data.frame(activity_subset = character(),
                                   p10 = numeric(),
                                   p50 = numeric(),
                                   p90 = numeric())
...
for (i in (1:length(activity_subsets))) {
  ...
  activity_subset_p10_p50_p90 <- data.frame(activity_subset = activity_subsets[i],
...
peer_agg_dist_summary <- peer_agg_mu_sigma_n |>
  dplyr::left_join(peer_agg_p10_p50_p90,
                   dplyr::join_by(activity_subset))
...
data_admission_avoidance <- peer_agg_ecdf_pdf |>
  dplyr::left_join(peer_agg_dist_summary,
                   dplyr::join_by(activity_subset)) |>
  dplyr::left_join(strategy_lookup,
                   dplyr::join_by(activity_subset == strategy)) |>
  dplyr::mutate(acivity_subset_label = paste(strategyLabel,
                                      ' (n = ',
                                      peers,
                                      ')',
                                      sep = '')) |>
...
get_probability_plot <- function(data, type) {
  if (type == "ecdf") {
    title <- "Emprirical cumulative distribition functions | aggregated peer opinions"
  ...
  plot <- data |>
    ...
    ggplot2::facet_wrap(ggplot2::vars(acivity_subset_label),
                        scale = 'free_y') +
